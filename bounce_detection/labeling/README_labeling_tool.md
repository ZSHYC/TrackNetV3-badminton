# 半自动标注工具

**Semi-automatic Labeling Tool for Bounce Detection**

基于 Phase 1 规则检测结果的交互式标注工具，用于生成 BounceNet 训练数据。

> 📖 相关文档:
>
> - [模块总览](../README.md)
> - [Phase 1 规则检测](../README_phase1_rules.md)
> - [Phase 2 BounceNet](../README_phase2_bouncenet.md)

---

## 📋 目录

- [功能特性](#功能特性)
- [快速开始](#快速开始)
- [使用指南](#使用指南)
- [界面说明](#界面说明)
- [快捷键](#快捷键)
- [数据格式](#数据格式)
- [批量标注](#批量标注)
- [导出训练数据](#导出训练数据)
- [API 参考](#api-参考)

---

## 功能特性

### 核心功能

- ✅ **Phase 1 预填充**: 自动运行规则检测，预填充候选事件
- ✅ **视频帧浏览**: 支持视频帧和轨迹同步显示
- ✅ **事件编辑**: 确认/删除/修改/添加事件
- ✅ **实时统计**: 显示标注进度和事件分布
- ✅ **自动保存**: JSON 格式标注文件
- ✅ **批量处理**: 支持整个 match 目录的连续标注
- ✅ **数据导出**: 导出为 CSV 训练格式
- ✅ **现代化界面**: 深色主题 + 清晰的视觉反馈

### 事件类型

| 类型   | 标识             | 符号 | 颜色      | 快捷键 |
| ------ | ---------------- | ---- | --------- | ------ |
| 落地点 | `landing`      | ★   | 🔴 珊瑚红 | L      |
| 击球点 | `hit`          | ◆   | 🔵 青绿色 | H      |
| 出画面 | `out_of_frame` | ✕   | ⬜ 灰色   | O      |
| 其他   | `other`        | ■   | 🟡 橙黄色 | N      |

---

## 快速开始

### 安装依赖

```bash
pip install numpy pandas matplotlib opencv-python
```

### 标注单个文件

```bash
# 基本用法（自动查找对应视频）
python labeling_launcher.py --csv data/test/match2/csv/1_04_04_ball.csv

# 指定视频文件
python labeling_launcher.py --csv data/test/match1/csv/1_05_02_ball.csv \
                            --video data/test/match1/video/1_05_02.mp4

# 加载已有标注继续编辑
python labeling_launcher.py --csv data/test/match1/csv/1_05_02_ball.csv \
                            --labels data/test/match1/labels/1_05_02_labels.json
```

### 批量标注

```bash
# 标注整个 match 目录
python labeling_launcher.py --match_dir data/test/match1
```

---

## 使用指南

### 工作流程

```
1. 启动工具
   └── 自动加载 CSV 轨迹
   └── 自动查找对应视频
   └── 自动运行 Phase 1 检测预填充
   
2. 浏览候选事件
   └── ↑/W 跳转到上一个事件
   └── ↓ 跳转到下一个事件
   └── ←/A 上一帧
   └── →/D 下一帧
   
3. 确认/修正事件
   └── Y 确认当前事件
   └── Delete 删除误检事件
   └── L/H/O 修改事件类型
   
4. 补充遗漏事件
   └── 导航到遗漏的帧
   └── N 添加新事件（默认为落地）
   └── L/H/O 设置类型
   
5. 保存标注
   └── S / Ctrl+S 保存
   └── Q / Esc 退出
```

### 标注标准

#### Landing (落地点) ★

- 球落地后停止或消失
- 典型特征：速度骤降至接近零

#### Hit (击球点) ◆

- 球被球拍击中的瞬间
- 典型特征：速度方向突然反转，速度大小保持较高

#### Out-of-frame (出画面) ✕

- 球飞出画面边缘
- 典型位置：画面顶部/底部/两侧边缘

---

## 界面说明

```
┌─────────────────────────────────────────────────────────────────────────┐
│                   🏸 羽毛球落点检测标注工具 v2.0                          │
├───────────────────────┬────────────┬────────────┬───────────────────────┤
│                       │            │            │                       │
│    视频帧 / 轨迹图     │  信息面板   │  事件列表  │    快捷键面板         │
│                       │            │            │                       │
│  - 当前帧画面          │ ╔═══════╗  │ ╔═══════╗  │  ╔═══════╗            │
│  - 轨迹线（渐变绿）    │ ║帧信息 ║  │ ║事件   ║  │  ║快捷键 ║            │
│  - 球位置（黄色光晕）  │ ╠═══════╣  │ ╠═══════╣  │  ╠═══════╣            │
│  - 事件标记           │ ║当前   ║  │ ║列表   ║  │  ║导航   ║            │
│    ★ 落地 ◆ 击打      │ ║事件   ║  │ ║       ║  │  ║编辑   ║            │
│    ✕ 出画 ■ 其他      │ ╠═══════╣  │ ║ ▶当前  ║  │  ║系统   ║            │
│                       │ ║统计   ║  │ ╚═══════╝  │  ╚═══════╝            │
│                       │ ╚═══════╝  │            │                       │
├───────────────────────┴────────────┴────────────┴───────────────────────┤
│                                                                         │
│     📈 Y 坐标变化         │      🚀 运动速度      │      按钮区域        │
│                                                                         │
├─────────────────────────────────────────────────────────────────────────┤
│                        状态栏: 准备就绪 | 按 Q 退出，S 保存              │
└─────────────────────────────────────────────────────────────────────────┘
```

### 图标说明

| 标记             | 含义           |
| ---------------- | -------------- |
| 🟡 黄色圆圈+光晕 | 当前帧球的位置 |
| ★ 红色星号      | 落地点事件     |
| ◆ 青绿菱形      | 击球点事件     |
| ✕ 灰色叉号      | 出画面事件     |
| ■ 橙黄方块      | 其他事件       |
| 实心图标         | 已确认事件     |
| 空心图标         | 未确认事件     |
| ▶ 箭头指示      | 当前选中的事件 |

---

## 快捷键

### 导航

| 快捷键         | 功能     | 说明                 |
| -------------- | -------- | -------------------- |
| `←` / `A` | 上一帧   | 后退一帧             |
| `→` / `D` | 下一帧   | 前进一帧             |
| `↑` / `W` | 上一事件 | 跳转到上一个检测事件 |
| `↓`         | 下一事件 | 跳转到下一个检测事件 |
| `Home`       | 第一帧   | 跳转到视频开头       |
| `End`        | 最后一帧 | 跳转到视频结尾       |
| `PageUp`     | 后退30帧 | 快速后退             |
| `PageDown`   | 前进30帧 | 快速前进             |

### 事件编辑

| 快捷键                     | 功能     | 说明                         |
| -------------------------- | -------- | ---------------------------- |
| `Y`                      | 确认事件 | 将当前事件标记为已确认       |
| `Delete` / `Backspace` | 删除事件 | 删除当前帧的事件             |
| `L`                      | 落地点   | 设置/添加为 Landing          |
| `H`                      | 击球点   | 设置/添加为 Hit              |
| `O`                      | 出画面   | 设置/添加为 Out-of-frame     |
| `N`                      | 新建事件 | 在当前帧添加事件（默认落地） |

### 系统

| 快捷键             | 功能 | 说明                 |
| ------------------ | ---- | -------------------- |
| `S` / `Ctrl+S` | 保存 | 保存标注到 JSON 文件 |
| `Q` / `Esc`    | 退出 | 退出程序（提示保存） |

> ⚠️ **注意**: `D` 键用于导航（下一帧），不是删除。删除请使用 `Delete` 或 `Backspace`。

---

## 数据格式

### 标注文件 (JSON)

```json
{
  "metadata": {
    "video_path": "data/test/match1/video/1_05_02.mp4",
    "csv_path": "data/test/match1/csv/1_05_02_ball.csv",
    "total_frames": 517,
    "fps": 30,
    "labeled_at": "2026-01-21T15:30:00"
  },
  "events": [
    {
      "frame": 30,
      "x": 481.0,
      "y": 385.0,
      "event_type": "hit",
      "rule": "vy_reversal",
      "confidence": 0.8,
      "confirmed": true,
      "features": {
        "vy_before": -21.0,
        "vy_after": 179.0
      }
    },
    {
      "frame": 516,
      "x": 652.0,
      "y": 698.0,
      "event_type": "landing",
      "rule": "trajectory_end",
      "confidence": 0.88,
      "confirmed": true,
      "features": {}
    }
  ]
}
```

> 注：`confidence` 由规则自动估计，不同规则的置信度区间不同，静止落地通常更高。

### 导出格式 (CSV)

```csv
rally,frame,x,y,event_type,confidence
1_05_02,30,481.0,385.0,hit,1.0
1_05_02,72,718.0,436.0,hit,1.0
1_05_02,516,652.0,698.0,landing,1.0
```

---

## 批量标注

### 批量标注流程

```bash
# 1. 启动批量标注
python labeling_launcher.py --match_dir data/test/match1

# 2. 自动显示待标注文件列表
# Found 11 CSV files
# Labeled: 0
# Pending: 11

# 3. 逐个标注
# - 每个文件标注完成后自动保存
# - 询问是否继续下一个

# 4. 标注结果保存到 data/test/match1/labels/
```

### 指定输出目录

```bash
python labeling_launcher.py --match_dir data/test/match1 \
                            --output_dir my_labels/match1
```

---

## 导出训练数据

### 导出所有已确认事件

```bash
python labeling_launcher.py --export data/test/match1/labels \
                            --output training_events.csv
```

### 查看标注统计

```bash
python labeling_launcher.py --stats data/test/match1/labels
```

输出示例：

```
============================================================
Labeling Statistics
============================================================
Total label files: 11
Total events: 101
Confirmed events: 85
Confirmation rate: 84.2%

By event type:
  hit: 62
  landing: 12
  out_of_frame: 11

By rally:
  1_05_02: 10/13 confirmed
  1_05_03: 8/11 confirmed
  ...
```

---

## API 参考

### LabelingDataManager

```python
from bounce_detection.labeling import LabelingDataManager

# 初始化
dm = LabelingDataManager(
    video_path='path/to/video.mp4',  # 可选
    csv_path='path/to/trajectory.csv',
    label_path='path/to/labels.json'  # 可选
)

# 访问数据
frame = dm.get_frame(100)           # 获取第 100 帧
x, y, vis = dm.get_trajectory_at_frame(100)  # 获取轨迹点

# 事件管理
dm.add_event({'frame': 100, 'event_type': 'hit'})
dm.confirm_event(100)
dm.update_event(100, {'event_type': 'landing'})
dm.remove_event(100)

# 保存
dm.save_labels('output.json')

# 统计
stats = dm.get_statistics()
```

### LabelingTool

```python
from bounce_detection.labeling import LabelingTool

# 初始化并启动
tool = LabelingTool(
    csv_path='path/to/trajectory.csv',
    video_path='path/to/video.mp4',  # 可选
    auto_detect=True  # 是否自动运行 Phase 1
)

# 启动交互界面
events = tool.run()
```

### LabelExporter

```python
from bounce_detection.labeling.data_manager import LabelExporter

# 导出为训练 CSV
LabelExporter.to_training_csv(dm, 'output.csv', only_confirmed=True)

# 导出事件列表
LabelExporter.to_event_list(dm, 'events.csv', only_confirmed=True)
```

---

## 文件结构

```
bounce_detection/labeling/
├── __init__.py              # 模块入口
├── data_manager.py          # 数据加载/保存/管理
├── visualizer.py            # 可视化组件
├── labeling_tool.py         # 主标注工具
└── README.md                # 本文档

labeling_launcher.py         # 启动脚本
test_labeling_tool.py        # 测试脚本
```

---

## 更新日志

### v0.1.0 (2026-01-21)

- ✅ 实现 LabelingDataManager 数据管理
- ✅ 实现 matplotlib 交互界面
- ✅ 支持 Phase 1 自动预填充
- ✅ 支持键盘快捷键操作
- ✅ 支持批量标注模式
- ✅ 支持 CSV/JSON 导出

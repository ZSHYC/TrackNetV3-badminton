# 羽毛球事件检测模块

**Badminton Event Detection Module**

检测羽毛球比赛中的关键事件：落地点 (Landing) 和击球点 (Hit)。

---

## 📋 模块概览

本模块是 TrackNetV3 的后处理扩展，基于轨迹坐标检测羽毛球的关键事件。

### 架构设计

```
┌─────────────────────────────────────────────────────────────────────────┐
│                    Bounce Detection Module                               │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│   Phase 1: 规则检测 (Rule-Based)          Phase 2: 深度学习分类          │
│   ┌─────────────────────────────┐        ┌─────────────────────────────┐│
│   │  KinematicsCalculator       │        │  VisualFeatureExtractor     ││
│   │  ├── 速度/加速度计算        │        │  ├── Patch 提取             ││
│   │  └── 运动方向/曲率          │        │  └── 运动历史图 (MHI)       ││
│   ├─────────────────────────────┤        ├─────────────────────────────┤│
│   │  BounceCandidateGenerator   │        │  BounceNet                  ││
│   │  ├── 7 条检测规则           │───────▶│  ├── 轨迹编码器 (LSTM)      ││
│   │  └── 候选事件生成           │        │  ├── 视觉编码器 (CNN)       ││
│   ├─────────────────────────────┤        │  └── 融合分类器             ││
│   │  BounceDetector             │        └─────────────────────────────┘│
│   │  └── 统一检测接口           │                                       │
│   └─────────────────────────────┘                                       │
│                                                                          │
│   辅助工具:                                                              │
│   ┌─────────────────────────────┐        ┌─────────────────────────────┐│
│   │  LabelingTool               │        │  BounceNetDataset           ││
│   │  └── 半自动标注界面         │        │  └── 训练数据加载           ││
│   └─────────────────────────────┘        └─────────────────────────────┘│
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
```

### 事件类型

| 事件类型 | 标识 | 描述 |
|---------|------|------|
| **落地点** | `landing` | 球落地后停止或消失 |
| **击球点** | `hit` | 球被球拍击中，方向反转 |
| **出画面** | `out_of_frame` | 球飞出画面边缘 |
| **非事件** | `none` | BounceNet 过滤的误检 |

---

## 📁 文件结构

```
bounce_detection/
├── README.md                    # 本文件 - 模块总览
├── README_phase1_rules.md       # Phase 1 规则检测详细文档
│
├── __init__.py                  # 模块入口，导出所有公共 API
├── kinematics.py                # 运动学特征计算
├── candidate_generator.py       # 规则检测 + 候选生成
├── detector.py                  # 统一检测器接口
│
├── visual_features.py           # 视觉特征提取 (Phase 2)
├── bouncenet.py                 # BounceNet 分类网络 (Phase 2)
├── dataset.py                   # 训练数据集 (Phase 2)
│
└── labeling/                    # 标注工具子模块
    ├── README_labeling_tool.md  # 标注工具详细文档
    ├── __init__.py
    ├── data_manager.py          # 数据管理
    ├── visualizer.py            # 可视化组件
    └── labeling_tool.py         # 标注工具主类
```

---

## 🚀 快速开始

### 安装依赖

```bash
pip install numpy pandas matplotlib torch opencv-python
```

### Phase 1: 规则检测

```python
from bounce_detection import BounceDetector

# 初始化检测器
detector = BounceDetector()

# 从 CSV 文件检测
events = detector.detect_from_csv('data/test/match1/csv/1_05_02_ball.csv')

for event in events:
    print(f"Frame {event['frame']:4d} | {event['event_type']:12s} | Rule: {event['rule']}")
```

### Phase 2: 带 BounceNet 的检测

```python
from bounce_detection import BounceDetector

# 加载带 BounceNet 的检测器
detector = BounceDetector(bouncenet_ckpt='ckpts/bouncenet/best.pt')

# 检测并过滤误检
events = detector.detect(x, y, visibility, use_bouncenet=True)
```

### 标注工具

```bash
# 单文件标注
python labeling_launcher.py --csv data/test/match1/csv/1_05_02_ball.csv

# 批量标注
python labeling_launcher.py --match_dir data/test/match1/csv
```

---

## 📖 详细文档

| 文档 | 内容 |
|------|------|
| [README_phase1_rules.md](README_phase1_rules.md) | Phase 1 规则检测：运动学计算、7条检测规则、参数配置 |
| [README_phase2_bouncenet.md](README_phase2_bouncenet.md) | Phase 2 BounceNet：网络架构、特征提取、训练流程 |
| [labeling/README_labeling_tool.md](labeling/README_labeling_tool.md) | 标注工具：界面使用、快捷键、数据格式 |

---

## 🔧 API 参考

### 核心类

```python
from bounce_detection import (
    # Phase 1
    BounceDetector,           # 统一检测器
    KinematicsCalculator,     # 运动学计算
    BounceCandidateGenerator, # 候选生成
    
    # Phase 2
    BounceNet,                # 分类网络
    BounceNetPredictor,       # 推理包装器
    VisualFeatureExtractor,   # 视觉特征
    
    # 常量
    EVENT_LABELS,             # {'none': 0, 'landing': 1, 'hit': 2}
    LABEL_TO_EVENT,           # {0: 'none', 1: 'landing', 2: 'hit'}
)
```

### BounceDetector

```python
class BounceDetector:
    def __init__(self,
                 fps: int = 30,
                 bouncenet_ckpt: str = None,    # BounceNet 检查点路径
                 use_visual_features: bool = False):
        ...
    
    def detect(self, x, y, visibility, 
               frames=None,              # 视频帧列表 (Phase 2)
               use_bouncenet=True) -> List[Dict]:
        """检测事件"""
    
    def detect_from_csv(self, csv_path) -> List[Dict]:
        """从 CSV 文件检测"""
```

### BounceNet

```python
class BounceNet:
    def __init__(self,
                 mode: str = 'trajectory_only',  # 'trajectory_only', 'visual_only', 'fusion'
                 num_classes: int = 3):
        ...
    
    def forward(self, traj_features, visual_features=None):
        """前向传播"""
    
    def predict(self, traj_features, visual_features=None) -> Dict:
        """预测，返回 labels, probs, confidence"""
```

---

## 📊 检测规则一览

| 规则 | 事件类型 | 置信度 | 描述 |
|------|---------|--------|------|
| `vy_reversal` | hit | 0.85 | Y方向速度反转 |
| `vx_reversal` | hit | 0.80 | X方向速度反转 |
| `acceleration_peak` | hit | 0.75 | 加速度突变 |
| `y_local_max` | hit | 0.60 | Y坐标局部极大值（最低点） |
| `speed_local_max` | hit | 0.60 | 速度局部极大值 |
| `visibility_drop` | landing | 0.85 | 场内可见性消失 |
| `speed_drop` | landing | 0.75 | 速度骤降 |
| `trajectory_end` | landing | 0.70 | 轨迹结束 |
| `visibility_drop_edge` | out_of_frame | 0.50 | 边缘可见性消失 |

---

## 🏋️ 训练 BounceNet

```bash
# 使用标注数据训练
python train_bouncenet.py \
    --label_dir labels/ \
    --mode trajectory_only \
    --epochs 100 \
    --batch_size 32 \
    --lr 1e-3

# 使用融合模式（需要视频）
python train_bouncenet.py \
    --label_dir labels/ \
    --mode fusion \
    --epochs 100
```

详见 [README_phase2_bouncenet.md](README_phase2_bouncenet.md)

---

## 📈 工作流程

```
1. TrackNetV3 检测          2. Phase 1 规则检测         3. 人工标注
   ┌─────────────┐            ┌─────────────┐            ┌─────────────┐
   │ Video       │───────────▶│ Candidates  │───────────▶│ Ground Truth│
   │ → CSV       │            │ (高召回率)   │            │ (JSON)      │
   └─────────────┘            └─────────────┘            └─────────────┘
                                                                │
                                                                ▼
4. 训练 BounceNet           5. Phase 2 检测             6. 最终结果
   ┌─────────────┐            ┌─────────────┐            ┌─────────────┐
   │ BounceNet   │◀───────────│ Candidates  │───────────▶│ Filtered    │
   │ 模型训练     │            │ + BounceNet │            │ Events      │
   └─────────────┘            └─────────────┘            └─────────────┘
```

---

## 📝 更新日志

- **v2.0** (2026-01-21): 添加 BounceNet 深度学习分类
  - 新增 `VisualFeatureExtractor` 视觉特征提取
  - 新增 `BounceNet` 分类网络（支持 trajectory/visual/fusion 三种模式）
  - 新增 `BounceNetDataset` 和训练脚本
  - 更新 `BounceDetector` 支持 BounceNet 集成

- **v1.2** (2026-01-22): 修正规则逻辑
  - 修正规则 6: `y_local_max` (Y坐标局部极大值/球最低点) 用于落地检测
  - 新增规则 5: `acceleration_peak` (加速度突变) 用于击球检测
  - 调整各规则置信度

- **v1.1** (2026-01-20): 增强规则检测
  - 新增规则: `speed_local_max` (速度局部极大值)
  - 候选召回率提升 ~29%

- **v1.0** (2026-01-19): 初始版本
  - Phase 1 规则检测实现
  - 半自动标注工具
